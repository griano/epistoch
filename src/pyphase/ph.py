...
class PhaseTypeGen(rv_continuous):
    ...
    def __init__(self, alpha, A, prec=None):
        alpha = ml.mat(alpha)
        A = ml.mat(A)
        if not CheckPHRepresentation(alpha, A, prec):
    ...
    def _get_alphaAi(self):
        # Compuates alpha * A^(-1), without actually inverting the matrix
        if self.alphaAi is None:
            self.alphaAi = np.linalg.solve(self.A.T, self.alpha.T).T
        return self.alphaAi
    
    def _get_erd(self):
        if self.erd is None:
            gamma = 1/self.mean()
            self.erd = phase(- gamma * self._get_alphaAi(), self.A)
        return self.erd
