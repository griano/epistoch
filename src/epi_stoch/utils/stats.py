...
def loss_function(dist, force=False):
    ...
    if not force:
        ...
        if name == "gamma":
            a = dist.kwds["a"]
            return_fun = lambda x: a * stats.gamma.sf(x, a=a + 1) - x * stats.gamma.sf(
                x, a
            )
        ...
        if name == "lognorm":
            ...
            def loss1(x):
                ...
                with warnings.catch_warnings():
                    ...
                    result = np.exp(0.5 * s * s) * stats.norm.sf(
                        np.log(x) / s - s
                    ) - x * stats.norm.sf(np.log(x) / s)
                return result
        ...
        if return_fun is not None:
            ...
            loss1 = lambda x: np.where(
                x > lo, scale * return_fun((x - loc) / scale), dist.mean() - x
            )
            return loss1
...
def test_loss_function():
    ...
    for force in [True, False]:
        ...
        np.testing.assert_allclose(
            loss_expo(x), loss_expo_at_x, atol=EPS, err_msg=f"Expo loss, force={force}"
        )
        np.testing.assert_allclose(
            loss_gm1(x), loss_expo_at_x, atol=EPS, err_msg=f"Gamma1 loss, force={force}"
        )
        ...
        loss_lnrm_at_x = np.array(
            [6, 5, 4, 3.00000000e000, 1.05078013e000, 7.72290351e-112]
        )
        assert loss_lnrm_at_x == pytest.approx(loss_lnrm(x), rel=EPS)
        loss_const_at_x = np.array([6.0, 5.0, 4.0, 3.0, 1.0, 0.0])
        np.testing.assert_allclose(
            loss_const(x), loss_const_at_x, err_msg=f"Constant loss, force={force}"
        )
